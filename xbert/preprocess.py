...
def convert_examples_to_features(
  examples, tokenizer, max_xseq_len=512, max_cseq_len=128,
...
def main(args):
    ...
    trn_features, xseq_lens, cseq_lens = convert_examples_to_features(
      trn_examples, tokenizer, args.max_xseq_len, args.max_cseq_len,
      pad_on_left=bool(args.model_type in ['xlnet']),
      pad_token=tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0],
      pad_token_segment_id=4 if args.model_type in ['xlnet'] else 0,
    )
    logger.info('trn_xseq: min={} max={} mean={} median={}'.format(
      np.min(xseq_lens), np.max(xseq_lens), np.mean(xseq_lens), np.median(xseq_lens)))
    logger.info('trn_cseq: min={} max={} mean={} median={}'.format(
      np.min(cseq_lens), np.max(cseq_lens), np.mean(cseq_lens), np.median(cseq_lens)))

    # create ttest set features
    tst_features, xseq_lens, cseq_lens = convert_examples_to_features(
      tst_examples, tokenizer, args.max_xseq_len, args.max_cseq_len,
      pad_on_left=bool(args.model_type in ['xlnet']),
      pad_token=tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0],
      pad_token_segment_id=4 if args.model_type in ['xlnet'] else 0,
    )
    logger.info('tst_xseq: min={} max={} mean={} median={}'.format(
      np.min(xseq_lens), np.max(xseq_lens), np.mean(xseq_lens), np.median(xseq_lens)))
    logger.info('tst_cseq: min={} max={} mean={} median={}'.format(
      np.min(cseq_lens), np.max(cseq_lens), np.mean(cseq_lens), np.median(cseq_lens)))
