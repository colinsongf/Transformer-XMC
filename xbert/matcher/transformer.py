...
class TransformerMatcher(object):
    ...
    def train(self, args, trn_features, eval_features=None, C_eval=None):
        ...
        all_attention_mask = torch.tensor(
            [f.attention_mask for f in trn_features], dtype=torch.long
        )
        all_token_type_ids = torch.tensor(
            [f.token_type_ids for f in trn_features], dtype=torch.long
        )
        all_output_ids = torch.tensor(
        ...
        if args.fp16:
            ...
            self.model, optimizer = amp.initialize(
                self.model, optimizer, opt_level=args.fp16_opt_level
            )
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if (
                        ...
                        eval_loss, eval_metrics, C_eval_pred, _ = self.predict(
                            args, eval_features, C_eval, topk=args.only_topk
                        )
                        logger.info("-" * 89)
                        logger.info(
